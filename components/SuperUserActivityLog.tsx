...
interface SuperUserActivityLogProps {
  logs: AuditLogEntry[];
  users: User[];
  entities: Entity[];
}
...
const SuperUserActivityLog: React.FC<SuperUserActivityLogProps> = ({ logs, users, entities }) => {
  ...
  const filteredLogs = useMemo(() => {
    const superAdminUserIds = new Set(superAdminUsers.map(u => u.id));
    return logs
      .filter(log => superAdminUserIds.has(log.userId))
      .filter(log => {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Registro de Actividad de Super Administradores</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
          ...
          <div>
            ...
            <select name="userId" value={filters.userId} onChange={handleFilterChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-teal-500 focus:border-teal-500">
              ...
              {superAdminUsers.map(user => (<option key={user.id} value={user.id}>{user.name}</option>))}
            </select>
          </div>
...
export default SuperUserActivityLog;